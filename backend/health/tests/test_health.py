...
class HealthCheckTests(APITestCase):
    def test_health_check_success(self):
        """
        Ensure the health check endpoint returns 200 OK and status 'ok' when DB is connected.
        """
    ...
    def test_health_check_db_failure(self, mock_cursor):
        """
        Ensure the health check endpoint returns 503 Service Unavailable when DB is disconnected.
        """
        # Mocking the cursor to raise OperationalError when accessed
        mock_cursor.side_effect = OperationalError("DB connection failed")
        
        # We need to patch where the code uses the connection. 
